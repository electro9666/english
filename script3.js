...
//	});
	...
	var youtubeapi = "resources/iframe_api.js";
	$(document).ready(function(){
		// left side
		...
		Handlebars.registerHelper('iff', function(parentIndex, index, options) {
			if (index == 0) {
...
//				]
		...
		var html = template(sentenceList);
		$("#leftSide").html(html);
		...
		if(ls != null && ls != ""){
			...
			var i = ls.indexOf("?f=");
			if(i != -1){
				var f = ls.substring(i+3);
				
				// 검증
				var isValidUrl = false;
				for(var i = 0 ; i<sentenceList.length ; i++){
					for(var j = 0 ; j<sentenceList[i].list.length ; j++){
						if(f == sentenceList[i].list[j]){
							isValidUrl = true;
							break;
						}
					}
				...
				$("#leftSide li a").each(function(i, d){
					if($(this).html() == f){
...
function onYouTubeIframeAPIReady() {
	if(typeof q == "undefined" || typeof list == "undefined"){
		$("#mainContents").hide();		
		$("#mainContents2").show();
		alert("데이터가 존재하지 않습니다. 화면을 새로고침 해주세요.");
		return;
	}
...
function repeatMaxUpdate(obj){
	...
	$("#btn0").html("리스트 각 " + repeatMax + "회 반복");
}
